...
cigar_ori = cv2.imread('cigar3.png',-1)
image = sys.argv[1]
...
faces=face_cascade.detectMultiScale(img, 1.2, 5, 0, (30, 30))
...
for (x, y, w, h) in faces:
    if h > 0 and w > 0:
            ...
            sh_glass = glass_symax - glass_min
 
            cigar_symin = int(y + 4 * h / 6)
            cigar_symax = int(y + 5.5 * h / 6)
            sh_cigar = cigar_symax - cigar_symin
 
            face_glass_roi_color = img[glass_min:glass_symax, x:x+w]
...
cv2.imwrite('output/image_re.png',img)
